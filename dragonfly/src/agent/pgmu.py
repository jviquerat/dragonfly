...
class pgmu(base_agent):
    def __init__(self, obs_dim, act_dim, n_cpu, size, pms):
        ...
        self.gnames = ["obs", "nxt", "act", "adv", "tgt", "lgp", "rwd", "trm"]
        self.gsizes = [obs_dim, obs_dim, self.pol_dim, 1, 1, 1, 1, 1]
    ...
    def train_critic(self, start, end):
        ...
        # Train v network
        #tgt = tf.reshape(tgt, [-1])
        self.v_net.loss.train(obs, nxt, rwd, trm,
                              self.retrn.gamma, self.v_net)
    ...
    def post_loop(self, style=None):
        ...
        self.gbuff.store(self.gnames, [gobs, gnxt, gact, gadv, gtgt, glgp, grwd, gtrm])
